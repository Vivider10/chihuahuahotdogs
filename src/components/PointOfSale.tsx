...
const menuItems: MenuItem[] = [
  ...
  { id: 'sprunk', name: 'Sprunk', description: 'Drink', price: 15, category: 'Drinks', color: 'sage', icon: Coffee },
  { id: 'e-cola', name: 'E-Cola', description: 'Drink', price: 15, category: 'Drinks', color: 'tomato', icon: Coffee },
  { id: 'slushie', name: 'Slushie', description: 'Drink', price: 18, category: 'Drinks', color: 'oat', icon: Sparkles },
...
export default function PointOfSale() {
  ...
  const completeSale = () => {
    ...
    setNotice(`Sale completed for ${currency.format(total)}${firstResponder ? ' · First responder discount applied' : ''}. Business totals updated.`)
    setCart([])
        ...
        <aside className="order-panel">
          ...
          <div className="checkout-area">
            <div className="totals">
              ...
              {cart.length > 0 && <button className={`discount-button ${firstResponder ? 'active' : ''}`} onClick={() => setFirstResponder((current) => !current)}><span>First Responder · 10%</span><strong>{firstResponder ? `−${currency.format(discount)}` : 'Apply'}</strong></button>}
              {firstResponder && <div><span>Discount</span><strong>−{currency.format(discount)}</strong></div>}
              <div className="grand-total"><span>Total</span><strong>{currency.format(total)}</strong></div>
            </div>
            <button className="charge-button" disabled={!cart.length} onClick={completeSale}><CreditCard size={21} /><span>Charge {currency.format(total)}</span><span className="arrow">→</span></button>
          </div>
        </aside>
      </div>
